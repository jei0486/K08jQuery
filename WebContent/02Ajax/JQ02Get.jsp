...
<script src="../common/jquery/jquery-3.5.1.js"></script>
    ...
    /*  
    $.get()
    	:HTTP Get방식을 통해 서버로부터 데이터를 받을떄
    	사용하는 메소드
    	
    	사용법
    		$.get(url, param, success(data));
    			-url : 정보를 요청할 경로
    			-param : 서버로 전송할 파라미터로 json 형태로 기술
    			-success(data) : 요청이 성공했을때 실행된 콜백(callback)메소드
    				콜백메소드가 호출될때 전달되는 파라미터를 통해 성공혹은 실패여부를 판단할 수 있다.
    ...
    */
			...
			$.get(
				...
				function(data) {
					...
					$(data).find("명함").each(function() {
						...
						$('#xmlDisplay').empty(html);
						$('#xmlDisplay').append(html);
						
						/*  
						//$('#xmlDisplay').html(html);
						위 두줄은 이 한줄코드랑 동일한 결과
						*/
					});	
				}
			);
        ...
        $('#btnJSP').click(function() {
			$.get(
				...
				,
				//콜백되는 메세지는 새 팝업창이 떴을 떄 페이지소스보기를이용하여
				//어떤 데이터가 콜백되는 지 확인할것
				//화면상에는 출력되지않지만 데이터에 영향을 끼치기도함
				function(data) {
					alert(data);
	...
	JSON으로 조립해준다.
	단 input요소의 name속성값이 key로 사용된다는 것에 주의하자
	*/
    function locationGo(link) {
		window.open(link,'','width=500,hight=500');
	}
